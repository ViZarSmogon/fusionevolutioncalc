...
function calculateSMSSSV(gen, attacker, defender, move, field) {
    ...
    (0, util_2.checkForestFury)(gen, defender, attacker);
    (0, util_2.checkForestFury)(gen, attacker, defender);
    (0, util_2.checkMadCow)(gen, attacker, defender);
    (0, util_2.checkMadCow)(gen, defender, attacker);
    (0, util_2.checkShockingFright)(gen, attacker, defender);
    (0, util_2.checkShockingFright)(gen, defender, attacker);
    (0, util_2.checkDauntingStorm)(gen, attacker, defender);
    (0, util_2.checkDauntingStorm)(gen, defender, attacker);
    (0, util_2.checkToxicAttitude)(gen, attacker, defender);
    (0, util_2.checkToxicAttitude)(gen, defender, attacker);
    ...
    (0, util_2.checkSquall)(attacker, field.attackerSide);
    (0, util_2.checkSquall)(defender, field.defenderSide);
    (0, util_2.checkStormClinic)(attacker, field.attackerSide);
    (0, util_2.checkStormClinic)(defender, field.defenderSide);
    if (move.named('Meteor Beam', 'Electro Shot')) {
        attacker.boosts.spa +=
            attacker.hasAbility('Simple') ? 2
                : attacker.hasAbility('Contrary', 'Unfiltered') ? -1
                    : 1;
    ...
    (0, util_2.checkOnceUponaTime)(attacker, field.defenderSide);
    (0, util_2.checkOnceUponaTime)(defender, field.attackerSide);
    var desc = {
    ...
    var breaksProtect = move.breaksProtect || move.isZ || attacker.isDynamaxed ||
        (attacker.hasAbility('Unseen Fist', 'Snowblind') && move.flags.contact);
    ...
    var defenderIgnoresAbility = defender.hasAbility('Full Metal Body', 'Prism Armor', 'Shadow Shield', 'Tablets of Ruin', 'Vessel of Ruin', 'Grindset', 'Sponge of Ruin', 'Lawnmower of Ruin');
    var attackerIgnoresAbility = attacker.hasAbility('Mold Breaker', 'Teravolt', 'Turboblaze', 'Molten Core', 'Circuit Breaker', 'Slushie', 'Bleeding Edge');
    ...
    var isCritical = !defender.hasAbility('Battle Armor', 'Shell Armor', 'No Nonsense') &&
    ...
    if (!move.isZ && !noTypeChange) {
        ...
        else if ((isMagnetize = attacker.hasAbility('Magnetize') && normal)) {
            type = 'Electric';
        }
        ...
        if (isGalvanize || isMagnetize || isPixilate || isRefrigerate || isAerilate || isNormalize) {
            desc.attackerAbility = attacker.ability;
    ...
    var isGhostRevealed = attacker.hasAbility('Scrappy') || attacker.hasAbility('Mind\'s Eye') || attacker.hasAbility('Scrap Rock') || attacker.hasAbility('Air Control') || attacker.hasAbility('Innovate') ||
    ...
    if ((defender.hasAbility('Wonder Guard') && typeEffectiveness <= 1) ||
        (move.hasType('Grass') && defender.hasAbility('Sap Sipper', 'Pondweed', 'Lawnmower of Ruin')) ||
        (move.hasType('Fire') && defender.hasAbility('Flash Fire', 'Well-Baked Body', 'Squall', 'Burning Petals', 'Well-Baked Flame Orb')) ||
        (move.hasType('Water') && defender.hasAbility('Dry Skin', 'Storm Drain', 'Water Absorb', 'Pondweed', 'Sponge of Ruin', 'Magnetic Storm', 'Daunting Storm')) ||
        (move.hasType('Electric') &&
            defender.hasAbility('Lightning Rod', 'Motor Drive', 'Volt Absorb', 'Electromagnetic Veil')) ||
        (move.hasType('Ground') &&
            !field.isGravity && !move.named('Thousand Arrows') &&
            !defender.hasItem('Iron Ball') && defender.hasAbility('Levitate', 'Holy Grail', 'Rising Tension', 'Free Flight', 'Airborne Armor', 'Overwhelming', 'Hellkite', 'Air Control', 'Magnetize', 'Unidentified Flying Object')) ||
        (move.flags.bullet && defender.hasAbility('Bulletproof')) ||
        (move.flags.sound && !move.named('Clangorous Soul') && defender.hasAbility('Soundproof')) ||
        (move.priority > 0 && defender.hasAbility('Queenly Majesty', 'Dazzling', 'Armor Tail', 'Armor Fist')) ||
        (move.hasType('Ground') && defender.hasAbility('Earth Eater')) ||
        (move.flags.wind && defender.hasAbility('Wind Rider', 'Storm Clinic')) ||
        (move.hasType('Ice') && defender.hasAbility('Squall')) ||
        (move.flags.punch && defender.hasAbility('Armor Fist'))) {
        desc.defenderAbility = defender.ability;
        return result;
    }
        ...
        for (stat in defender.boosts) {
            if (defender.boosts[stat] > 0) {
                attacker.boosts[stat] +=
                    attacker.hasAbility('Contrary', 'Unfiltered') ? -defender.boosts[stat] : defender.boosts[stat];
    ...
    var applyBurn = attacker.hasStatus('brn') &&
        ...
        !attacker.hasAbility('Guts', 'Well-Baked Flame Orb') &&
        !move.named('Facade');
    ...
    if ((move.dropsStats && move.timesUsed > 1) || move.hits > 1) {
        ...
        var _loop_1 = function (times) {
            ...
            hasAteAbilityTypeChange = hasAteAbilityTypeChange &&
                attacker.hasAbility('Aerilate', 'Galvanize', 'Magnetize', 'Pixilate', 'Refrigerate', 'Normalize');
            if ((move.dropsStats && move.timesUsed > 1)) {
...
function calculateBPModsSMSSSV(gen, attacker, defender, move, field, desc, basePower, hasAteAbilityTypeChange, turnOrder) {
    ...
    if ((attacker.hasAbility('Technician', 'Stone Wheel', 'Bleeding Edge', 'Hydrotechnic') && basePower <= 60) ||
        ...
        (attacker.hasAbility('Sharpness', 'Rebel\'s Blade', 'Ancient Marble') && move.flags.slicing)) {
        bpMods.push(6144);
        desc.attackerAbility = attacker.ability;
    }
    ...
    var isFieldAuraShield = field.isAuraShield && move.secondaries;
    var auraActive = isAttackerAura || isDefenderAura || isFieldFairyAura || isFieldDarkAura || isFieldAuraShield;
    ...
    if ((attacker.hasAbility('Sheer Force', 'Sand Wrath', 'Force of Nature', 'Overwhelming') &&
    ...
    if ((attacker.hasAbility('Reckless', 'Reachless') && (move.recoil || move.hasCrashDamage)) ||
        (attacker.hasAbility('Iron Fist', 'Armor Punch') && move.flags.punch) ||
        (attacker.hasAbility('Armor Punch') && move.priority > 0)) {
        bpMods.push(4915);
        desc.attackerAbility = attacker.ability;
    }
    ...
    if (gen.num <= 8 && defender.hasAbility('Heatproof', 'Heatproof Drive') && move.hasType('Fire')) {
        bpMods.push(2048);
        desc.defenderAbility = defender.ability;
    }
    ...
    if (attacker.hasAbility('Supreme Overlord', 'Emperor\'s Clothes') && attacker.alliesFainted) {
...
function calculateAttackSMSSSV(gen, attacker, defender, move, field, desc, isCritical) {
    ...
    else if (defender.hasAbility('Unaware', 'Eczema', 'Numbskull')) {
        attack = attackSource.rawStats[attackStat];
        desc.defenderAbility = defender.ability;
    }
    else {
    ...
    if (attacker.hasAbility('Hustle', 'Grindset') && move.category === 'Physical') {
...
function calculateAtModsSMSSSV(gen, attacker, defender, move, field, desc) {
    ...
    if ((attacker.hasAbility('Slow Start') && attacker.abilityOn &&
        ...
        (attacker.hasAbility('Firewall') && move.hasType('Fire') && (attacker.hasItem('Booster Energy') || field.hasTerrain('Electric')))) {
        atMods.push(6144);
        desc.attackerAbility = attacker.ability;
    }
    else if (attacker.hasAbility('Flash Fire', 'Burning Petals', 'Suppressive Fire') && attacker.abilityOn && move.hasType('Fire')) {
        atMods.push(6144);
        desc.attackerAbility = attacker.ability;
    }
    else if ((attacker.hasAbility('Steelworker') && move.hasType('Steel')) ||
        (attacker.hasAbility('Dragon\'s Maw', 'Hellkite') && move.hasType('Dragon')) ||
        (attacker.hasAbility('Rocky Payload', 'Riot Payload') && move.hasType('Rock')) ||
        (attacker.hasAbility('Necromancer') && move.hasType('Ghost'))) {
        atMods.push(6144);
        desc.attackerAbility = attacker.ability;
    }
    ...
    else if (attacker.hasAbility('Galvanic Relay') && move.hasType('Electric')) {
        atMods.push(5325);
        desc.attackerAbility = attacker.ability;
    }
    ...
    if ((defender.hasAbility('Thick Fat', 'Fat Fingers') && move.hasType('Fire', 'Ice')) ||
        (defender.hasAbility('Water Bubble') && move.hasType('Fire')) ||
        (defender.hasAbility('Purifying Salt') && move.hasType('Ghost')) ||
        (defender.hasAbility('Necromancer') && (move.hasType('Ghost') || move.hasType('Steel')))) {
        atMods.push(2048);
        desc.defenderAbility = defender.ability;
    }
    if (gen.num >= 9 && defender.hasAbility('Heatproof', 'Heatproof Drive') && move.hasType('Fire')) {
        atMods.push(2048);
        desc.defenderAbility = defender.ability;
    }
    ...
    var isGrindsetActive = (defender.hasAbility('Grindset') || field.isGrindset) &&
        !attacker.hasAbility('Grindset');
    var isLawnmowerOfRuinActive = (defender.hasAbility('Lawnmower of Ruin') || field.isLawnmowerOfRuin) &&
        !attacker.hasAbility('Lawnmower of Ruin');
    ...
    else if ((isGrindsetActive && move.category === 'Physical')) {
        if (defender.hasAbility('Grindset')) {
            desc.defenderAbility = defender.ability;
        }
        else {
            desc['isGrindset'] = true;
        }
        atMods.push(2048);
    }
    else if ((isLawnmowerOfRuinActive && move.category === 'Special')) {
        if (defender.hasAbility('Lawnmower of Ruin')) {
            desc.defenderAbility = defender.ability;
        }
        else {
            desc['isLawnmowerOfRuin'] = true;
        }
        atMods.push(3072);
    }
    if ((0, util_2.isQPActive)(attacker, field)) {
        ...
        else if (attacker.hasAbility('Ultra Impulse') && attacker.hasStatus('brn') &&
            ((move.category === 'Physical' && (0, util_2.getQPBoostedStat)(attacker) === 'atk') ||
                (move.category === 'Special' && (0, util_2.getQPBoostedStat)(attacker) === 'spa'))) {
            atMods.push(6144);
            desc.attackerAbility = attacker.ability;
        }
    ...
    if ((attacker.hasAbility('Hadron Engine') && move.category === 'Special' &&
        ...
        (attacker.hasAbility('Iron Sights') && (move.category === 'Special' || move.category === 'Physical')) ||
        (attacker.hasAbility('Mercury Pulse') && move.category === 'Physical' &&
            field.hasWeather('Rain', 'Heavy Rain') && !attacker.hasItem('Utility Umbrella'))) {
        atMods.push(5461);
        desc.attackerAbility = attacker.ability;
    }
...
function calculateDefenseSMSSSV(gen, attacker, defender, move, field, desc, isCritical) {
    ...
    else if (attacker.hasAbility('Unaware', 'Eczema', 'Numbskull')) {
        defense = defender.rawStats[defenseStat];
        desc.attackerAbility = attacker.ability;
    }
    else {
...
function calculateDfModsSMSSSV(gen, attacker, defender, move, field, desc, isCritical, hitsPhysical) {
    ...
    var isSpongeOfRuinActive = (attacker.hasAbility('Sponge of Ruin') || field.isSpongeOfRuin) &&
        !defender.hasAbility('Sponge of Ruin');
    ...
    else if (isSpongeOfRuinActive && !hitsPhysical) {
        if (attacker.hasAbility('Sponge of Ruin')) {
            desc.attackerAbility = attacker.ability;
        }
        else {
            desc['isSpongeOfRuin'] = true;
        }
        dfMods.push(3072);
    }
    if ((0, util_2.isQPActive)(defender, field)) {
        ...
        else if (defender.hasAbility('Ultra Impulse') && defender.hasStatus('brn') &&
            ((move.category === 'Physical' && (0, util_2.getQPBoostedStat)(defender) === 'atk') ||
                (move.category === 'Special' && (0, util_2.getQPBoostedStat)(defender) === 'spa'))) {
            dfMods.push(6144);
            desc.defenderAbility = defender.ability;
        }
    }
...
function calculateFinalModsSMSSSV(gen, attacker, defender, move, field, desc, isCritical, typeEffectiveness, hitCount) {
    ...
    else if (attacker.hasAbility('Tinted Lens', 'Sharp Goggles') && typeEffectiveness < 1) {
        finalMods.push(8192);
        desc.attackerAbility = attacker.ability;
    }
    else if (attacker.hasAbility('Unfiltered') && typeEffectiveness < 1) {
        finalMods.push(5461);
        desc.attackerAbility = attacker.ability;
    }
    ...
    if (defender.hasAbility('Multiscale', 'Shadow Shield', 'Steamy Scales') &&
        defender.curHP() === defender.maxHP() &&
        hitCount === 0 &&
        (!field.defenderSide.isSR && (!field.defenderSide.spikes || defender.hasType('Flying')) ||
    ...
    if (defender.hasAbility('Fluffy') && move.flags.contact && !attacker.hasAbility('Long Reach', 'Fat Fingers')) {
        finalMods.push(2048);
        desc.defenderAbility = defender.ability;
    }
    ...
    if (defender.hasAbility('Solid Rock', 'Filter', 'Prism Armor', 'Unfiltered', 'Scrap Rock', 'Airborne Armor', 'Suppressive Fire') && typeEffectiveness > 1) {
        finalMods.push(3072);
        desc.defenderAbility = defender.ability;
    }
    if (defender.hasAbility('Shear Strength') && move.secondaries) {
        finalMods.push(3072);
        desc.defenderAbility = defender.ability;
    }
